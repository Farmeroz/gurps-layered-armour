...
test('Hardened belongs to its own layer; order does not change basic penetration', () => {
  const outer = layer(12, { hardened: 1 }),
    inner = layer(6);
  for (const layers of [
    [outer, inner],
    [inner, outer],
...
test('Hardened progression, ignore DR, fractions and unsupported divisors', () => {
  ...
  assert.equal(hardenedDivisor(0.5, 6), 0.5);
  assert.throws(() => hardenedDivisor(7, 1));
  assert.equal(
    stackFor(profile(layer(30), layer(100, { hardened: 1 })), 'Torso', 'pi', -1).effectiveDR,
    1,
  );
});
test('combined rounding preserves native additive DR for layers sharing a divisor', () => {
  ...
  assert.deepEqual(
    stackFor(p, 'Torso', 'cut', 2).rows.map((r) => r.effective),
    [0, 1],
  );
  assert.equal(stackFor(profile(layer(0)), 'Torso', 'cr', 0.5).effectiveDR, 2);
});
test('zero split DR, explicit locations, disabled coverage and uncovered fallback', () => {
  const p = profile(
    layer(9, {
      split: { cr: 0, cut: 4 },
      locations: [{ where: 'Torso', dr: 3, split: { cut: 7 } }],
    }),
  );
  ...
  p.enabled = false;
  assert.equal(stackFor(p, 'Torso', 'imp'), null);
});
...
test('consecutive flexible layers combine for blunt trauma; types and penetration respected', () => {
  const stack = stackFor(
    profile(layer(6, { flexible: true }), layer(10, { flexible: true })),
    'Torso',
    'cr',
  );
...
test('validation rejects malformed flags and ambiguous slash DR; forcefields cannot be flexible', () => {
  ...
  for (const dr of [-1, NaN, Infinity, '4/2', '', 0.5])
    assert.throws(() => validateProfile(profile(layer(dr))));
  assert.deepEqual(parseSplit('cr=0; pi+=8'), { cr: 0, 'pi+': 8 });
  assert.throws(() => parseSplit('pi=4; pi=7'));
  assert.throws(() => parseSplit('__proto__=7'));
  assert.equal(
    validateProfile(profile(layer(2, { kind: 'forcefield', flexible: true }))).layers[0].flexible,
    false,
  );
