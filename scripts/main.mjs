...
export async function openEditor(actor, options = {}) {
  try {
    if (!canEdit(actor, game.user))
      throw new Error('Choose an actor you own. GMs can edit any actor.');
    ...
    if (!options.temporary && editors.get(key)?.rendered) {
      editors.get(key).bringToTop();
...
export async function open(options = {}) {
  if (Object.hasOwn(options, 'actor') || Object.hasOwn(options, 'token'))
    return openEditor(options.actor ?? options.token?.actor);
  ...
  if (!actors.length) {
    ui.notifications.warn(
      'Armour Layers: select a token, use an actor’s directory menu, or use /armour "Actor Name".',
    );
    return null;
  }
  const results = [];
...
export async function command(line) {
  ...
  if (/^(?:help|\?)$/i.test(name)) {
    ui.notifications.info(
      '/armour ["Actor Name"]: opens Armour Layers for selected tokens, your assigned character, or a named world actor. /armor is an alias.',
      { permanent: true },
    );
    return true;
  }
  ...
  if (actors.length !== 1) {
    ui.notifications.warn(
      actors.length
        ? 'More than one owned actor has that name. Use its directory menu.'
        : 'No owned actor has that exact name.',
    );
    return true;
  }
...
Hooks.once('ready', async () => {
  ...
  if (registry?.registerProcessor) {
    ...
    const aliases = ['armour', 'armor'].filter((alias) => {
      ...
      if (conflict)
        ui.notifications.warn(
          `Armour Layers: /${alias} is already registered. Use the directory menu or module API.`,
        );
      return !conflict;
    });
    if (aliases.length)
      registry.registerProcessor({
        ...
        usagematches: () => false,
        help: () => `/${aliases[0]} ["Actor Name"]: edit Armour Layers`,
        isGMOnly: () => false,
        process: command,
      });
  } else
    ui.notifications.warn(
      'Armour Layers: chat integration is unavailable. Use the actor menu or token HUD.',
    );
  try {
    if (!/^0\.18\./.test(game.system.version))
      throw new Error('ADD integration requires GGA 0.18.x. The editor remains available.');
    const NativeADD =
  ...
  Hooks.on('updateActor', (actor, changes) => {
    if (!changes.flags?.[ID] && !Object.keys(changes).some((key) => key.startsWith(`flags.${ID}`)))
      return;
