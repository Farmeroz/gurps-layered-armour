import {readProfile, validateProfile, stackFor, traceDamage, canEdit, escapeHTML as esc, elementOf} from './core.mjs';
...
export function stateFor(dialog) {
  ...
  if (!calc || !Array.isArray(calc._calculators)) throw new Error('Unsupported GGA damage calculator.');
  function wrap(object, key, replacement) {
    ...
    Object.defineProperty(object, key, {configurable: true, get() {return replacement.call(this, () => descriptor.get.call(this));},
  ...
  for (const child of calc._calculators) {
    wrap(child, 'calculatedBluntTrauma', native => {
      const {stack} = report(state);
      return stack ? traceDamage(stack, child.effectiveDamage, calc.damageType).bluntTrauma : native();
    });
  }
  return state;
}
export function report(state) {
  if (!state.useLayers) return {status: 'Native DR selected for this ADD. Review the DR override and native armour options.'};
  try {
    const profile = validateProfile(state.override ?? readProfile(state.dialog.actor));
    if (!profile.enabled) return {status: 'No active armour profile. The ADD uses normal sheet DR.'};
    const calc = state.dialog._calculator;
    if (['Large-Area', 'Random', 'User Entered'].includes(calc.hitLocation)) {
      return {error: 'Choose a specific hit location, or turn off layered DR for this ADD and enter reviewed native DR. Automatic large-area and explosion armour calculations are not included in this release.'};
    }
    let location = calc.hitLocation;
    const localised = globalThis.game?.i18n?.localize?.(`GURPS.hitLocation${location}`);
    if (!profile.layers.some(l => l.allLocations || l.locations.some(x => x.where === location)) && localised) location = localised;
    const divisor = !calc.useArmorDivisor || calc.isExplosion ? 1 : (calc.armorDivisor || 1);
    const multiplier = calc.isShotgun && calc.shotgunRofMultiplier > 1 ? calc.shotgunDamageMultiplier : 1;
    const stack = stackFor(profile, location, calc.damageType, divisor, multiplier);
    return stack ? {stack, status: `${state.override ? 'This ADD’s temporary' : 'Actor’s saved'} layers replace sheet DR at ${calc.hitLocation}.`} :
...
export function reviewError(state) {
  ...
  const calc = state.dialog._calculator;
  const children = calc.viewId === 'all' ? calc._calculators : [calc._calculators[Number(calc.viewId)]];
...
export function reportHTML(state) {
  const result = report(state), calc = state.dialog._calculator;
  if (!result.stack) return `<p>${esc(result.error ?? result.status)}</p>`;
  const children = calc.viewId === 'all' ? calc._calculators : [calc._calculators[Number(calc.viewId)]];
  return `<div class="armour-report"><p>${esc(result.status)}</p><p><strong>DR ${result.stack.rawDR}; effective DR ${result.stack.effectiveDR}</strong>. Hardened applies per layer. Wounding follows penetration.</p>` + children.filter(Boolean).map((child, index) => {
    const trace = traceDamage(result.stack, child.effectiveDamage, calc.damageType);
    return `<p>Hit ${calc.viewId === 'all' ? index + 1 : Number(calc.viewId) + 1}: ${child.effectiveDamage} ${esc(calc.damageType)} → ${trace.penetrating} penetrating; calculated blunt trauma ${trace.bluntTrauma}${child._bluntTrauma !== null ? ` (override ${child._bluntTrauma})` : ''}.</p>
      <table><thead><tr><th>Outer → inner</th><th>DR</th><th>Hard.</th><th>Divisor</th><th>Effective DR*</th><th>Damage in → out</th></tr></thead><tbody>` + trace.rows.map(row => `<tr><td>${esc(row.name)}${row.flexible ? ' (flexible)' : ''}</td><td>${row.dr}</td><td>${row.hardened}</td><td>${row.divisor === -1 ? '∞' : row.divisor}</td><td>${row.effective}</td><td>${row.incoming} → ${row.outgoing}</td></tr>`).join('') + `</tbody></table>${trace.review ? `<p>${esc(trace.review)}</p>` : ''}`;
  }).join('') + '<p>*Fractional protection is retained across layers and the total is rounded down once. Rows allocate the rounded protection in layer order.</p></div>';
}
...
function fixResults(root, state) {
  ...
  if (raw?.nextElementSibling?.nextElementSibling) raw.nextElementSibling.nextElementSibling.textContent = `${calc.hitLocation}: configured armour layers`;
  if ((calc.useArmorDivisor && calc.armorDivisor) || calc.isShotgun) {
    ...
    if (formula) formula.textContent = '= sum of layer DR after each layer’s divisor; round total down';
  }
}
export function patchADD(NativeADD, openEditor) {
  ...
  const nativeGetData = proto.getData, nativeListeners = proto.activateListeners;
  ...
  proto.activateListeners = function (html) {
    ...
    const state = stateFor(this), root = elementOf(html);
    ...
    const current = report(state), review = reviewError(state);
    ...
    panel.querySelector('[data-use-layers]').addEventListener('change', ev => {state.useLayers = ev.currentTarget.checked;this.render(false);});
    for (const button of panel.querySelectorAll('[data-edit]')) {
      button.disabled = !canEdit(this.actor, game.user);
      button.addEventListener('click', () => {
        if (button.dataset.edit === 'reset') {state.override = null;state.useLayers = true;this.render(false);return;}
        const temporary = button.dataset.edit === 'temporary';
        void openEditor(this.actor, {temporary, ...(temporary ? {profile: state.override ?? readProfile(this.actor)} : {}),
          onSave: value => {if (temporary) state.override = value;state.useLayers = true;this.render(false);}});
      });
    }
    if (current.stack) {
      for (const input of root.querySelectorAll('#override-dr input, #override-dr button, #hardened, [name="hardened"], #flexible-armor')) {
        input.disabled = true;input.title = 'Controlled by Armour Layers. Turn off layered DR above to use native controls.';
      }
      ...
      for (const input of root.querySelectorAll('input[name="hitlocation"]')) {
        ...
        if (value && row?.nextElementSibling?.nextElementSibling) row.nextElementSibling.nextElementSibling.textContent = String(value.rawDR);
      }
  ...
  proto.resolveInjury = async function (keepOpen, injury, publicly, results = null) {
    ...
    if (results !== null) {
      ...
      if (error) {ui.notifications.error(`Armour Layers: ${error}`);throw new Error(error);}
      if (!canEdit(this.actor, game.user)) throw new Error('You no longer have permission to apply damage to this actor.');
      if (report(state).stack) {
        const holder = document.createElement('div');holder.innerHTML = results;fixResults(holder, state);
