...
export function locationsOf(actor) {
  const names = [...new Set(Object.values(actor.system?.hitlocations ?? {}).map(x => x.where).filter(Boolean))];
  return names.length ? names : ['Torso'];
}
function number(value, label, max = 1000000) {
  if (value === '' || value === null || !Number.isFinite(Number(value)) || Number(value) < 0 || Number(value) > max || !Number.isInteger(Number(value)))
...
export function parseSplit(text) {
  ...
  for (const term of text.split(';')) {
    ...
    if (!match || !TYPES.includes(match[1])) throw new Error('Split DR uses entries such as cut=4; pi=6; pi+=6. Enter each damage type explicitly.');
    if (Object.hasOwn(result, match[1])) throw new Error(`Duplicate split DR type: ${match[1]}.`);
    result[match[1]] = number(match[2], 'Split DR');
  }
  return result;
}
export const formatSplit = value => Object.entries(value ?? {}).map(([key, dr]) => `${key}=${dr}`).join('; ');
function split(value) {
  if (!value || Array.isArray(value) || typeof value !== 'object') throw new Error('Invalid split DR.');
  return Object.fromEntries(Object.entries(value).map(([key, dr]) => {
...
export function validateProfile(value) {
  ...
  if (!Array.isArray(value.layers) || value.layers.length > 100) throw new Error('Use at most 100 layers.');
  return {schema: 1, enabled: !!value.enabled, layers: value.layers.map(layer => {
    const name = String(layer.name ?? '').trim();
    if (!name || name.length > 100) throw new Error('Each layer needs a name of up to 100 characters.');
    if (!['worn', 'natural', 'forcefield'].includes(layer.kind)) throw new Error('Unknown layer kind.');
    if (!Array.isArray(layer.locations) || layer.locations.length > 200) throw new Error('Invalid location coverage.');
    const seen = new Set();
    return {name, enabled: !!layer.enabled, kind: layer.kind, dr: number(layer.dr, `${name} DR`),
      hardened: number(layer.hardened, `${name} Hardened`, 6), flexible: layer.kind === 'forcefield' ? false : !!layer.flexible,
      split: split(layer.split), allLocations: !!layer.allLocations,
      locations: layer.locations.map(loc => {
        if (typeof loc.where !== 'string' || !loc.where.trim() || loc.where.length > 100 || seen.has(loc.where)) throw new Error('Invalid or duplicate hit location.');
        seen.add(loc.where);
        return {where: loc.where, dr: loc.dr === null ? null : number(loc.dr, 'Location DR'), split: split(loc.split)};
      })};
  })};
}
export function newLayer(locations) {
  return {name: 'New armour', enabled: true, kind: 'worn', dr: 0, hardened: 0, flexible: false, split: {}, allLocations: false,
    locations: [{where: locations.includes('Torso') ? 'Torso' : locations[0], dr: null, split: {}}]};
}
export const covers = (layer, where) => layer.allLocations || layer.locations.some(loc => loc.where === where);
...
export function hardenedDivisor(divisor, level) {
  ...
  if (index < 0) throw new Error(`Divisor ${divisor} is outside the Hardened progression. Review this hit with native DR.`);
...
export function stackFor(profile, where, type, divisor = 1, multiplier = 1) {
  ...
  if (!(divisor > 0 || divisor === -1) || !Number.isFinite(divisor)) throw new Error('Invalid armour divisor.');
  if (!Number.isFinite(multiplier) || multiplier < 1) throw new Error('Invalid shotgun multiplier.');
  let exactTotal = 0, rawTotal = 0;
  const rows = profile.layers.filter(layer => layer.enabled && covers(layer, where)).map(layer => {
    ...
    return {name: layer.name, kind: layer.kind, flexible: layer.flexible, hardened: layer.hardened,
      dr, divisor: effectiveDivisor, exact, effective: Math.floor(exactTotal + 1e-9) - before};
  });
  ...
  if (rawTotal === 0 && divisor < 1 && divisor > 0) {
    ...
    rows.push({name: 'DR 0 vs fractional divisor (B379)', kind: 'natural', flexible: false, hardened: 0, dr: 0, divisor, exact: effectiveDR, effective: effectiveDR});
  }
  return {rows, rawDR: rawTotal, effectiveDR};
}
export function traceDamage(stack, damage, type) {
  let remaining = damage, flexIncoming = null, stopped = null;
  ...
  const threshold = type === 'cr' ? 5 : ['cut', 'imp', 'pi-', 'pi', 'pi+', 'pi++'].includes(type) ? 10 : 0;
  let bluntTrauma = 0, review = '';
  if (stopped?.row.flexible && threshold) {
    ...
    if (bluntTrauma && stack.rows.slice(index + 1).some(r => !r.flexible && r.effective > 0)) {
      review = 'Flexible armour stopped the attack before an inner rigid layer. Enter a reviewed blunt-trauma value in the ADD before applying.';
    }
  }
